      ...
      <div className={`${styles.boxWidth} mx-auto`}>
        ...
        >
          <input
            ...
            onChange={(e) => setQuery(e.target.value)}
            placeholder="Search financial videos..."
            className="p-3 w-full sm:w-3/4 rounded bg-[#1A1A1A] text-white border border-gray-700 focus:outline-none"
          />

          <motion.button
            onClick={fetchVideos}
            className="py-3 px-6 text-black bg-gray-300 rounded-[10px]"
            whileHover={{ scale: 1.05 }}
            whileTap={{ scale: 0.95 }}
          >
            Search
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-12">
          {videos.map((video, index) => (
            <motion.div
              key={index}
              className="bg-[#111] border border-gray-700 p-4 rounded-[20px]"
              whileHover={{ scale: 1.05 }}
            >
              <h3 className="text-gray-100 mb-3">{video.title}</h3>
              <iframe
                width="100%"
                height="180"
                src={`https://www.youtube.com/embed/${video.videoId}`}
        ...
        <h2 className="text-center text-gray-200 text-3xl mb-8">
          Trending Financial News
        </h2>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
          {trendingNews.map((news, index) => (
            <motion.div
              ...
              whileHover={{ scale: 1.05 }}
            >
              <h3 className="text-gray-100 mb-2">{news.title}</h3>
              <p className="text-gray-400 mb-3">{news.summary}</p>
              <a
                href={news.url}
                target="_blank"
                rel="noopener noreferrer"
                className="inline-block py-2 px-4 bg-gray-300 text-black rounded-[10px]"
              >
                Read More
              </a>
            </motion.div>
