...
async def get_historical_price(coin_id: str, vs_currency: str = "usd", days: int = 365):
    ...
    return clean_dict([{"date": row["date_str"], "price": row["price"]} for _, row in df.iterrows()])
...
async def get_performance_heatmap(coin_id: str, vs_currency: str = "usd", days: int = 365):
    ...
    return clean_dict(heatmap.to_dict(orient="records"))
...
async def monte_carlo_prediction(coin_id: str, vs_currency: str = "usd", num_simulations: int = 1000, days: int = 252):
    ...
    expected_price = float(np.nanmean(simulations[:, -1]))
    prob_positive = float(np.nanmean(simulations[:, -1] > last_price))
    ...
    for i in range(min(4, num_simulations)):
        simulation_paths.append({
            ...
            "data": [{"day": day, "value": float(simulations[i, day]) if not np.isnan(simulations[i, day]) else None} for day in range(0, days, 5)]
        })
